...
#page = requests.get(url2, headers = headers, params = params)
for page_no in range(1,93): 
   ...
   #dataset = [(x.text, y.text) for x,y in zip(grave_name, dates)]
   #print(dataset)

#with open('Fernandiabeach3.csv', 'w',) as csvfile:
#    writer = csv.writer(csvfile)
...
#with open('Fernandiabeach.csv', 'w',  encoding ='utf8',  newline= '') as f:
#    thewriter = writer(f)
#    for list in graves:
#        dates = graves.find('b', {'class':'birthDeathDates'}).text
#        cemetary = graves.find('button', {'role': 'link'}).text
#        name = graves.find('h2', {'class':'name-grave'}).text
#        info = [title, cemetary, name, info]
